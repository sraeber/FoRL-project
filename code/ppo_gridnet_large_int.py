...
if __name__ == "__main__":
    ...
    for update in range(starting_update, args.num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            #create reward weights for this iteration:
            shift = 1000000
            adapt = 3000000
            if global_step <= shift:
                new_weight = envs.reward_weight
            else:
                new_weight = [linear_schedule(envs.reward_weight[0],float(1.0),adapt,global_step-shift)]
                for r in envs.reward_weight[1:]:
                    new_weight.append(linear_schedule(r,float(0.0),adapt,global_step-shift))
            new_weight = torch.tensor(new_weight).double()
            rs = torch.matmul(raw_rewards,new_weight)
            rs = rs.float()
            rewards[step] = rs.to(device)
            next_done =  torch.Tensor(ds).to(device)

            for info in infos:
                if "episode" in info.keys():
                    print(f"global_step={global_step}, episodic_return={info['episode']['r']},rewards = {new_weight}")
